...
class FastRRTStarPlanner:
    ...
    for collisions.
    """
    def __init__(self, costmap: list[int], origin_x: float, origin_y: float, resolution: float, columns: int, rows: int, max_access_cost: int, max_iterations: int = 10000, min_iterations: int = 200, expansion_radius: float = 5.0, search_radius: float = 2.0, dichotomy_distance: float = 0.05, tolerance_distance: float = 0.25):
        ...
        self.x_upper_bound = self.costmap_origin_x_ + self.costmap_cols_ * self.costmap_resolution_
        self.y_lower_bound = self.costmap_origin_y_
        self.y_upper_bound = self.costmap_origin_y_ + self.costmap_rows_ * self.costmap_resolution_

    def sample_free_node(self):
        return Node(random.uniform(self.x_lower_bound, self.x_upper_bound), random.uniform(self.y_lower_bound, self.y_upper_bound))
    
    def get_nearest_node(self, node: Node) -> int:
        # Query the KDTree for the nearest node based on Euclidean distance
        self.tree.query((node.position_x, node.position_y))
        nearest_index = self.tree.query((node.position_x, node.position_y))[1]
        return nearest_index

    def get_nearby_nodes(self, node: Node) -> list[int]:
        # Query the KDTree for all nodes within the search radius
        indices = self.tree.query_ball_point((node.position_x, node.position_y), self.search_radius_)
        return indices
    ...
    def is_collision_free(self, node1: Node, node2: Node) -> bool:
        ...
        for col, row in points:
            if col < 0 or col >= self.costmap_cols_ or row < 0 or row >= self.costmap_rows_:
                return False
    ...
    def create_node(self, reachest_node: Node, candidate_node: Node) -> Node:
        ...
        if allow_node != reachest_node: # Was using "is" which is incorrect, checks for reference equality
    ...
    def distance(self, node1: Node, node2: Node) -> float:
        return ((node1.position_x - node2.position_x) ** 2 + (node1.position_y - node2.position_y) ** 2) ** 0.5
    ...
    def steer(self, nearest_node: Node, candidate_node: Node) -> Node:
        ...
        theta = atan2(candidate_node.position_y - nearest_node.position_y, candidate_node.position_x - nearest_node.position_x)
    ...
    def has_initial_plan(self, goal_node: Node) -> bool:
        within_goal_threshold = self.distance(self.nodes[-1], goal_node) <= self.tolerance_distance_
    ...
    def rewire(self, candidate_node: Node, nearby_nodes_indices: list[int]):
        for index in nearby_nodes_indices:
            ...
            rewire_cost = candidate_node.cost + candidate_node.get_connection_cost(nearby_node)
            if rewire_cost < nearby_node.cost and self.is_collision_free(candidate_node, nearby_node):
                nearby_node.update_parent_and_cost(candidate_node)
    
    def make_plan(self, start_x: float, start_y: float, goal_x: float, goal_y: float) -> list[Node]:
        ...
        self.tree = KDTree([(start_node.position_x, start_node.position_y)])
        ...
        for i in range(self.max_iterations_):
            ...
            if self.is_collision_free(nearest_node, candidate_node):
                ...
                # Rewire
                self.rewire(candidate_node, nearby_nodes_indices) # We can use the "old" indices because candidate_node is added at the end of self.nodes
                self.tree = KDTree([(node.position_x, node.position_y) for node in self.nodes])
